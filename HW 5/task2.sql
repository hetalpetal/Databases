...
SELECT SUM(bill_total) FROM restBill WHERE bill_date='February 2016';
SELECT AVG(bill_total) FROM restBill WHERE table_no='2';
SELECT MIN(no_of_seats) FROM restRest_table WHERE room_name='Blue';
SELECT MAX(no_of_seats) FROM restRest_table WHERE room_name='Blue';
SELECT AVG(no_of_seats) FROM restRest_table WHERE room_name='Blue';
SELECT COUNT(table_no) FROM restBill WHERE waiter_no='002' AND '004';
